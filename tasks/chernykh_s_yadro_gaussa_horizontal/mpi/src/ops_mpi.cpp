...
void GaussFilter(const std::vector<int>& local_data, std::vector<int>& local_res, int local_rows, int width, int extended_rows, int halo_top){
        ...
        for (int j = 0; j < width; ++j) {
            ...
            for (int ki = -1; ki <= 1; ++ki) {
                for (int kj = -1; kj <= 1; ++kj) {
                    ...
                    int weight;
                    if (ki == 0 && kj == 0) weight = 4;
                    else if (ki == 0 || kj == 0) weight = 2;
                    else weight = 1;
...
bool ChernykhSYadroGaussaHorizontalMPI::RunImpl() {
  ...
  MPI_Scatterv(data_to_send, send_counts_ptr, new_start_ptr, MPI_INT, local_data.data(), extended_rows * w, MPI_INT, 0, MPI_COMM_WORLD);
  ...
  MPI_Gatherv(local_res.data(),local_rows * w,MPI_INT,final_output_ptr,recvcounts_ptr,recvdispls_ptr,MPI_INT,0, MPI_COMM_WORLD);
